...
public class RequestJsonHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private Object bindRequestParameters(MethodParameter parameter, NativeWebRequest webRequest) throws Exception {
        ...
        if (StringUtils.isNotEmpty(key)) {
            ...
            if (value == null) {
                if (parameterAnnotation.required()) {
                    ...
                    Class<?> parameterType = parameter.getParameterType();
                    if (isBasicDataTypes(parameterType) || isBaseType(parameterType) || isStringDataTypes(parameterType)) {
                        if(isBooleanDataTypes(parameterType) || isBooleanType(parameterType)){
                            value = false;
                        }else if(isStringDataTypes(parameterType)){
                            value = "";
                        }else {
                            value = 0;
                        }
            ...
            // 由于以json方式入参时，long/Long类型数值会默认转换为int/Integer，故在此先做转换
            if(parameterType.equals(Long.class) || parameterType.getName().equals("long")){
                if(value ==null){
                    value = 0;
                }
                Long valueTemp = Long.valueOf(value.toString());
                if (valueTemp == null) {
                    if (parameterAnnotation.required()) {
                        throw new IllegalArgumentException(String.format("required param %s is not present", key));
                    } else {
        ...
        if (value != null) {
            if (isBasicDataTypes(parameterType) || isStringDataTypes(parameterType)) {
                return value;
            }
    ...
    @SuppressWarnings("rawtypes")
    private boolean isBooleanDataTypes(Class clazz) {
    ...
    @SuppressWarnings("rawtypes")
    private boolean isStringDataTypes(Class clazz) {
        Set<Class> classSet = new HashSet<>();
    ...
    public void contributeMethodArgument(MethodParameter parameter, Object value,
        ...
        RequestJson ann = parameter.getParameterAnnotation(RequestJson.class);
        String name = (ann == null || !org.springframework.util.StringUtils.hasText(ann.value()) ? parameter.getParameterName() : ann.value());
